...
# Reúne as soluções dos exercícios e projetos da aula.
# O foco é a busca sequencial e operações sobre vetores não ordenados.
...
def busca_sequencial(lista, alvo):
    ...
    for i in range(len(lista)):
        if lista[i] == alvo:
            # Retorna a primeira posição encontrada.
            return i

    # -1 indica que o valor não está presente.
    return -1


def conta_ocorrencias(valor, lista):
    # Conta todas as aparições do valor, por isso percorre a lista inteira.
    contador = 0
...
def remove_elemento(lista, valor):
    # Uma lista vazia não possui elemento que possa ser removido.
    if len(lista) == 0:
        return None

    # Guarda a posição da primeira ocorrência encontrada.
    indice = -1
    ...
    # Valor ausente: a lista permanece sem alteração.
    if indice == -1:
        return None

    # Guarda o elemento que será devolvido ao final da operação.
...
def buscar_cliente(nome, lista_clientes):
    # Reaproveita a busca sequencial para localizar o cliente.
    return busca_sequencial(lista_clientes, nome)


def buscar_filme(catalogo, titulo):
    # Procura o título e usa o índice para informar o resultado.
...
def verificar_convidado(lista_convidados, nome):
    # O resultado da busca determina se o convidado está cadastrado.
...
if __name__ == "__main__":
    # Exemplos dos exercícios de busca sequencial.
    lista = [7, 12, 5, 12, 8]
    ...
    # Exemplos de contagem de ocorrências.
    ...
    # Exemplo de remoção da primeira ocorrência.
